...
from scene.er_tcnet import ERTCNet
from scene.gwformer_encoder import GWformerEncoder
...
class SongUNet(nn.Module):
    def __init__(self,
        ...
        gwformer_patch_size = 4,
        gwformer_embed_dim  = 96,
        gwformer_depths     = (2, 2, 6, 2),
        gwformer_num_heads  = (3, 6, 12, 24),
        gwformer_window_size = 8,
        gwformer_mlp_ratio  = 4.0,
        pdc_types           = ("cpdc", "apdc", "hpdc", "vpdc"),
        use_wavelet_attention = True,
        use_window_attention = True,
        er_embed_dim        = 0,            # ER-TCNet embed dim, 0 = disabled
    ):
        assert embedding_type in ['fourier', 'positional']
        assert encoder_type in ['standard', 'skip', 'residual', 'gwformer']
        ...
        self.er_embed_dim = er_embed_dim
        if emb_dim_in > 0:
        ...
        self.encoder_skip_specs = [
            (int(name.split('x')[0]), block.out_channels)
            for name, block in self.enc.items() if 'aux' not in name
        ]
        skips = [channels for _, channels in self.encoder_skip_specs]
        ...
        if self.encoder_type == 'gwformer':
            self.gwformer_encoder = GWformerEncoder(
                img_resolution=img_resolution,
                in_channels=in_channels,
                patch_size=gwformer_patch_size,
                embed_dim=gwformer_embed_dim,
                depths=gwformer_depths,
                num_heads=gwformer_num_heads,
                window_size=gwformer_window_size,
                mlp_ratio=gwformer_mlp_ratio,
                pdc_types=pdc_types,
                use_wavelet_attention=use_wavelet_attention,
                use_window_attention=use_window_attention,
            )
            self.gwformer_stage_channels = [
                gwformer_embed_dim,       # 128×128
                gwformer_embed_dim,       # 64×64
                gwformer_embed_dim,       # 32×32
                gwformer_embed_dim * 2,   # 16×16
                gwformer_embed_dim * 4,   # 8×8
                gwformer_embed_dim * 8,   # 4×4
            ]
            self.gwformer_stage_resolutions = [
                img_resolution,           # 128×128
                img_resolution // 2,      # 64×64
                img_resolution // gwformer_patch_size,           # 32×32
                img_resolution // (gwformer_patch_size * 2),     # 16×16
                img_resolution // (gwformer_patch_size * 4),     # 8×8
                img_resolution // (gwformer_patch_size * 8),     # 4×4
            ]

            # GWformer adapters — output channels match CNN skip channels
            # so GWformer features can be ADDED to CNN skips
            self.gw_skip_source_indices = []
            self.gw_skip_adapters = nn.ModuleList()
            for target_res, target_channels in self.encoder_skip_specs[:-1]:
                source_idx = self._select_gwformer_stage(target_res)
                self.gw_skip_source_indices.append(source_idx)
                self.gw_skip_adapters.append(
                    nn.Conv2d(
                        in_channels=self.gwformer_stage_channels[source_idx],
                        out_channels=target_channels,
                        kernel_size=1,
                    )
                )

            bottleneck_channels = self.encoder_skip_specs[-1][1]
            self.gwformer_bottleneck_adapter = nn.Conv2d(
                in_channels=self.gwformer_stage_channels[-1],
                out_channels=bottleneck_channels,
                kernel_size=1,
            )

            # Gated fusion: per-channel learnable gate for each skip resolution
            self.gw_gates = nn.ParameterList()
            for target_res, target_channels in self.encoder_skip_specs:
                self.gw_gates.append(nn.Parameter(torch.full((1, target_channels, 1, 1), -3.0)))

            # Zero-init adapters so GWformer contribution starts at zero
            for adapter in self.gw_skip_adapters:
                nn.init.zeros_(adapter.weight)
                nn.init.zeros_(adapter.bias)
            nn.init.zeros_(self.gwformer_bottleneck_adapter.weight)
            nn.init.zeros_(self.gwformer_bottleneck_adapter.bias)

            # GWformer runs in parallel — CNN encoder is still active and trainable

        # ER-TCNet skip injection (gated fusion, same pattern as GWformer)
        if er_embed_dim > 0:
            self.er_skip_adapters = nn.ModuleList()
            self.er_gates = nn.ParameterList()
            for target_res, target_channels in self.encoder_skip_specs:
                self.er_skip_adapters.append(
                    nn.Conv2d(er_embed_dim, target_channels, kernel_size=1)
                )
                self.er_gates.append(nn.Parameter(torch.full((1, target_channels, 1, 1), -3.0)))
            for adapter in self.er_skip_adapters:
                nn.init.zeros_(adapter.weight)
                nn.init.zeros_(adapter.bias)

    def _select_gwformer_stage(self, target_res):
        target_res = max(int(target_res), 1)
        dists = [abs(np.log2(max(int(res), 1)) - np.log2(target_res)) for res in self.gwformer_stage_resolutions]
        return int(np.argmin(dists))

    def _augment_skips_with_gwformer(self, x, cnn_skips):
        """Run GWformer in parallel and ADD its features to CNN skip connections."""
        gw_features = self.gwformer_encoder(x)

        for idx, (target_res, _) in enumerate(self.encoder_skip_specs):
            if idx == len(self.encoder_skip_specs) - 1:
                gw_feat = self.gwformer_bottleneck_adapter(gw_features[-1])
            else:
                source_idx = self.gw_skip_source_indices[idx]
                gw_feat = self.gw_skip_adapters[idx](gw_features[source_idx])

            if gw_feat.shape[-2] != target_res or gw_feat.shape[-1] != target_res:
                gw_feat = torch.nn.functional.interpolate(
                    gw_feat,
                    size=(target_res, target_res),
                    mode='bilinear',
                    align_corners=False,
                )
            # Gated fusion: CNN skip + gate * GWformer feature
            gate = torch.sigmoid(self.gw_gates[idx])
            cnn_skips[idx] = cnn_skips[idx] + gate * gw_feat

        return cnn_skips

    def _augment_skips_with_er(self, er_features, cnn_skips):
        """Inject ER-TCNet spatial features into skip connections with gated fusion."""
        for idx, (target_res, _) in enumerate(self.encoder_skip_specs):
            er_feat = self.er_skip_adapters[idx](er_features)
            if er_feat.shape[-2] != target_res or er_feat.shape[-1] != target_res:
                er_feat = torch.nn.functional.interpolate(
                    er_feat, size=(target_res, target_res),
                    mode='bilinear', align_corners=False,
                )
            gate = torch.sigmoid(self.er_gates[idx])
            cnn_skips[idx] = cnn_skips[idx] + gate * er_feat
        return cnn_skips

    def forward(self, x, film_camera_emb=None, N_views_xa=1, er_features=None):
        ...
        # Encoder.
        # Save original input for parallel GWformer branch
        original_input = x
        # Always run CNN encoder (it's the primary path)
        skips = []
        aux = x
        ...
        # If GWformer is enabled, run it in parallel on the ORIGINAL input and augment CNN skips
        if self.encoder_type == 'gwformer':
            skips = self._augment_skips_with_gwformer(original_input, skips)

        # If ER-TCNet features are provided, inject into skips with gated fusion
        if er_features is not None:
            skips = self._augment_skips_with_er(er_features, skips)
...
class SingleImageSongUNetPredictor(nn.Module):
    def __init__(self, cfg, out_channels, bias, scale, er_embed_dim=0):
        ...
        encoder_type = getattr(cfg.model, "encoder_type", "unet")
        ...
        self.encoder = SongUNet(cfg.data.training_resolution,
                                ...
                                gwformer_patch_size=getattr(cfg.model, "gwformer_patch_size", 4),
                                gwformer_embed_dim=getattr(cfg.model, "gwformer_embed_dim", 96),
                                gwformer_depths=tuple(getattr(cfg.model, "gwformer_depths", [2, 2, 6, 2])),
                                gwformer_num_heads=tuple(getattr(cfg.model, "gwformer_num_heads", [3, 6, 12, 24])),
                                gwformer_window_size=getattr(cfg.model, "gwformer_window_size", 8),
                                gwformer_mlp_ratio=getattr(cfg.model, "gwformer_mlp_ratio", 4.0),
                                pdc_types=tuple(getattr(cfg.model, "pdc_types", ["cpdc", "apdc", "hpdc", "vpdc"])),
                                use_wavelet_attention=getattr(cfg.model, "use_wavelet_attention", True),
                                use_window_attention=getattr(cfg.model, "use_window_attention", True),
                                er_embed_dim=er_embed_dim)
...
class GaussianSplatPredictor(nn.Module):
    def __init__(self, cfg):
        ...
        # Pre-compute ER-TCNet status before creating networks
        er_cfg = getattr(cfg.model, "er_tcnet", None)
        self.er_tcnet_enabled = er_cfg is not None and getattr(er_cfg, "enabled", False)
        self.er_embed_dim = int(getattr(er_cfg, "embed_dim", 128)) if self.er_tcnet_enabled else 0

        if cfg.model.network_with_offset:
            split_dimensions, scale_inits, bias_inits = self.get_splits_and_inits(True, cfg)
            er_dim = self.er_embed_dim
        ...
        if self.er_tcnet_enabled:
            if cfg.model.network_with_offset:
                channel_dim = sum(self.split_dimensions_with_offset)
            else:
                channel_dim = sum(self.split_dimensions_without_offset)

            # ER-TCNet processes the original image (3 channels), not network_output
            er_spatial_size = 8
            er_input_dim = 3 * er_spatial_size * er_spatial_size  # 3 * 64 = 192
            self.er_spatial_size = er_spatial_size

            self.er_tcnet = ERTCNet(
                img_global_dim=er_input_dim,
                n_points=getattr(er_cfg, "n_points", 256),
                sample_points=getattr(er_cfg, "sample_points", 64),
                point_hidden_dim=getattr(er_cfg, "point_hidden_dim", 256),
                point_feat_dim=getattr(er_cfg, "point_feat_dim", 64),
                dgrb_radius=getattr(er_cfg, "dgrb_radius", 0.1),
                dgrb_n=getattr(er_cfg, "dgrb_n", 10),
                dgrb_m=getattr(er_cfg, "dgrb_m", 1),
                dgrb_k=getattr(er_cfg, "dgrb_k", 8),
                embed_dim=getattr(er_cfg, "embed_dim", 128),
                cap_dim=getattr(er_cfg, "cap_dim", 64),
                num_heads=getattr(er_cfg, "num_heads", 4),
                lambda_reg=getattr(er_cfg, "lambda_reg", 0.05),
                n_iter=getattr(er_cfg, "n_iter", 3),
                fast_sampling=getattr(er_cfg, "fast_sampling", True),
            )
            self.er_spatial_grid = 8  # reshape 64 capsules to 8×8 spatial
            self.er_channel_dim = channel_dim
    ...
    def _set_main_network_trainable(self, is_trainable):
        if hasattr(self, "network_with_offset"):
            self._set_module_trainable(self.network_with_offset, is_trainable)
        if hasattr(self, "network_wo_offset"):
            self._set_module_trainable(self.network_wo_offset, is_trainable)

    def _freeze_er_unused_heads(self):
        if not self.er_tcnet_enabled:
            return
        if hasattr(self.er_tcnet, "er_routing") and hasattr(self.er_tcnet.er_routing, "activation_conv"):
            self._set_module_trainable(self.er_tcnet.er_routing.activation_conv, False)
    ...
    def set_training_stage(self, stage):
        ...
        if stage == 1:
            self._set_main_network_trainable(False)
            self._set_module_trainable(self.er_tcnet, False)
            self._set_module_trainable(self.er_tcnet.point_head, True)
            self._set_module_trainable(self.er_tcnet.dgrb, True)
        elif stage == 2:
            self._set_main_network_trainable(False)
            self._set_module_trainable(self.er_tcnet, True)
            self._set_module_trainable(self.er_tcnet.point_head, False)
            self._set_module_trainable(self.er_tcnet.dgrb, False)
        else:
            self._set_main_network_trainable(True)
            self._set_module_trainable(self.er_tcnet, True)

        self._freeze_er_unused_heads()

    def _run_er_tcnet_encoder(self, x, B, N_views):
        """Run ER-TCNet on original input, return spatial features for skip injection + aux outputs."""
        if not self.er_tcnet_enabled:
            return None, None

        # Pool input to small spatial size for ER-TCNet
        f_spatial = F.adaptive_avg_pool2d(x, (self.er_spatial_size, self.er_spatial_size))
        f_spatial = f_spatial.reshape(B, N_views, -1).mean(dim=1)  # [B, er_input_dim]

        er_out = self.er_tcnet(f_spatial)

        # F_pc: [B, num_caps, embed_dim] → reshape to spatial [B*N, embed_dim, grid, grid]
        F_pc = er_out["F_pc"]  # [B, 64, embed_dim]
        grid = self.er_spatial_grid  # 8
        er_features = F_pc.reshape(B, grid, grid, self.er_embed_dim)
        er_features = er_features.permute(0, 3, 1, 2)  # [B, embed_dim, 8, 8]
        er_features = er_features.unsqueeze(1).expand(B, N_views, -1, -1, -1)
        er_features = er_features.reshape(B * N_views, self.er_embed_dim, grid, grid)

        return er_features, er_out
    ...
    def forward(self, x, 
        ...
        # Run ER-TCNet on original input to produce spatial features for skip injection
        er_features = None
        if self.er_tcnet_enabled:
            er_features, er_aux = self._run_er_tcnet_encoder(x, B, N_views)
        ...
        if return_aux:
            aux_dict = {}
            if er_aux is not None:
                aux_dict = {
                    "er_P0": er_aux["P0"],
                    "er_C": er_aux["C"],
                    "er_a_L": er_aux["a_L"],
                    "er_F_pc": er_aux["F_pc"],
                    "er_P_hat": er_aux["P_hat"],
                }
