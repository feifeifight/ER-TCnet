...
from scene.er_tcnet import entropy_regularization_loss, point_cloud_regularization_loss
...
def main(cfg: DictConfig):
    ...
    er_enabled = er_cfg is not None and getattr(er_cfg, "enabled", False)
    if er_enabled and hasattr(gaussian_predictor, "set_training_stage"):
        gaussian_predictor.set_training_stage(getattr(er_cfg, "training_stage", 3))

    trainable_params = [p for p in gaussian_predictor.parameters() if p.requires_grad]
    optimizer = torch.optim.Adam(trainable_params, lr=cfg.opt.base_lr, eps=1e-15,
                                 betas=cfg.opt.betas)
    ...
    lambda_er = float(getattr(cfg.opt, "lambda_er", 0.0))
    lambda_pc = float(getattr(cfg.opt, "lambda_pc", 0.0))
    ...
    for num_epoch in range((cfg.opt.iterations + 1 - first_iter)// len(dataloader) + 1):
        ...
        for data in dataloader:
            ...
            if er_enabled:
                gaussian_splats, aux_outputs = gaussian_predictor(
                    input_images,
            ...
            er_loss_sum = torch.zeros((), device=total_loss.device)
            pc_reg_loss = torch.zeros((), device=total_loss.device)
            if er_enabled and len(aux_outputs) > 0:
                if lambda_er > 0 and "er_C" in aux_outputs:
                    er_loss_sum = entropy_regularization_loss(aux_outputs["er_C"])
                if lambda_pc > 0 and "er_P0" in aux_outputs:
                    pc_reg_loss = point_cloud_regularization_loss(aux_outputs["er_P0"])
                total_loss = total_loss + lambda_er * er_loss_sum + lambda_pc * pc_reg_loss
            ...
            if iteration % 100 == 0:
                loss_str = "iter {} | loss {:.4f}".format(iteration, total_loss.item())
                if er_enabled and len(aux_outputs) > 0:
                    loss_str += " | er {:.4f} | pc_reg {:.4f}".format(er_loss_sum.item(), pc_reg_loss.item())
                print(loss_str)
            ...
            with torch.no_grad():
                if iteration % cfg.logging.loss_log == 0 and fabric.is_global_zero:
                    ...
                    if er_enabled and len(aux_outputs) > 0:
                        wandb.log({"training_er_loss": np.log10(er_loss_sum.item() + 1e-8)}, step=iteration)
                        wandb.log({"training_pc_reg_loss": np.log10(pc_reg_loss.item() + 1e-8)}, step=iteration)
                    if cfg.data.category == "hydrants" or cfg.data.category == "teddybears":
